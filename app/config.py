...
class Settings(BaseModel):
    ...
    # -------------------------------------------------
    # Risk-7: Jobs quota / anti-abuse
    # -------------------------------------------------
    jobs_rate_limit_per_minute: int = Field(default_factory=lambda: int(os.getenv("JOBS_RATE_LIMIT_PER_MINUTE", "10")))
    jobs_max_active_per_user: int = Field(default_factory=lambda: int(os.getenv("JOBS_MAX_ACTIVE_PER_USER", "3")))
    jobs_max_payload_bytes: int = Field(default_factory=lambda: int(os.getenv("JOBS_MAX_PAYLOAD_BYTES", str(256 * 1024))))  # 256KB
